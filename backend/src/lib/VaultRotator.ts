/**
 * Vault Rotator - Key Rotation Manager
...
import ApexVault, { DecryptionContext, EncryptedPayload } from './ApexVault';
...
export class VaultRotator {
  ...
  public async initiateRotation(oldMasterKey: string, newMasterKey: string): Promise<void> {
    ...
    console.log('[Vault-Rotation] Initiating zero-downtime key rotation...');

    try {
      // Fire off background rotation
      this.rotateAllResourcesBackground(oldMasterKey, newMasterKey).catch((err) => {
        console.error('[Vault-Rotation] Background rotation failed:', err);
  ...
  private async rotateAllResourcesBackground(
    ...
    console.log('[Vault-Rotation] Starting batch rotation process...');
    ...
    while (true) {
      ...
      console.log(
        `[Vault-Rotation] Processing batch: records ${offset} - ${offset + records.length}`
      );
    ...
    console.log(
      `[Vault-Rotation] Rotation complete: ${successful} successful, ${failed} failed`
    );
    console.log(`[Vault-Rotation] Total time: ${duration}min`);
    console.log(`[Vault-Rotation] Success rate: ${metrics.successRate}%`);
  ...
  private async rotateRecord(
    ...
    try {
      ...
      if (!decryptResult.success || !decryptResult.data) {
        console.error(`[Vault-Rotation] Failed to decrypt record ${record.id}`);
        return false;
      }
      ...
      if (!encryptResult.success || !encryptResult.data) {
        console.error(`[Vault-Rotation] Failed to re-encrypt record ${record.id}`);
        return false;
      }
      ...
      console.log(`[Vault-Rotation] ✓ Successfully rotated record ${record.id}`);
      return true;
    } catch (err) {
      console.error(`[Vault-Rotation] ✗ Rotation failed for record ${record.id}:`, err);
      return false;
    }
  }
  ...
  public async getRotationStatus(): Promise<RotationStatus> {
    ...
    const progress = totalRecords > 0 ? ((rotatedRecords / totalRecords) * 100).toFixed(2) : '0';
  ...
  public async verifyRotation(masterKey: string, sampleSize: number = 10): Promise<boolean> {
    console.log(`[Vault-Rotation] Verifying rotation with ${sampleSize} random samples...`);
    ...
    for (const record of records) {
      ...
      if (result.success && result.data) {
        ...
        console.error(`[Vault-Rotation] Verification failed for record ${record.id}`);
      }
    }

    const verificationPass = verifiedCount === sampleSize;
    console.log(
      `[Vault-Rotation] Verification: ${verifiedCount}/${sampleSize} records decryptable`
    );
  ...
  public async markRotationFailed(): Promise<void> {
    console.warn('[Vault-Rotation] CRITICAL: Marking rotation as failed. Reverting to old key...');

    this.rotationInProgress = false;

    // In production: trigger alert, notify security team, etc.
    console.warn('[Vault-Rotation] Alert: Rotation failure detected. Manual intervention required.');
  }
  ...
  public scheduleRotation(
    ...
    console.log(
      `[Vault-Rotation] Scheduling rotation in ${(delayMs / 1000 / 60).toFixed(2)} minutes...`
    );

    return setTimeout(() => {
      this.initiateRotation(oldMasterKey, newMasterKey).catch((err) => {
        console.error('[Vault-Rotation] Scheduled rotation failed:', err);
      });
    }, delayMs);
  }
