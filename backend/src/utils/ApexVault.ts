/**
 * ApexVault Vault Integration for HAVEN
 * Replaces basic prismaVault.ts with production-grade encryption
 * 
 * This module integrates ApexVault into HAVEN's case management system,
...
export class ApexVault {
  ...
  private logAudit(event: string, context: object): void {
    ...
    console.log(`[APEXVAULT-AUDIT] ${JSON.stringify(auditEntry)}`);
  }
}
...
if (!masterKey) {
  throw new Error(
    'VAULT_KEY or VAULT_MASTER_KEY environment variable is required. Generate with: openssl rand -hex 32'
  );
}
